            ...
            if commande == "REQ_LIST_IDS":
                ...
                try:
                    ...
                    if message_clair.startswith("CMD_FINAL"):
                        ...
                        print(f"Oignon ouvert : Instruction CMD_RELAY -> Vers ID {prochaine_id}")
                        
                        prochaine_ip = demander_ip_au_master(master_ip, prochaine_id)
                        if prochaine_ip != "ERROR":
                            try:
                                prochaine_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                                prochaine_socket.connect((prochaine_ip, Port_Routeur))
                            ...
                            print(f"ID {prochaine_id} inconnu du Master")
